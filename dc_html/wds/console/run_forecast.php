...
try {
  ...
  $days = isset($_POST['days']) ? max(1, min(16, (int)$_POST['days'])) : 16;
...
<!doctype html>
<html lang="zh-CN">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>拉取预报 · 结果</title><link rel="stylesheet" href="/wds/assets/css/console.css"></head>
<body>
  <div class="wrap">
    <div class="card">
      <h2 style="font-size:20px">✅ 预报拉取成功</h2>
      <p class="muted">版本 <?=$version?> · 本次更新地点数：<b><?=$count?></b></p>
      <?php if ($count): ?>
      ...
      <?php endif; ?>
      <p style="margin-top:12px"><a href="/wds/"><button class="btn-daily" type="button">返回控制台</button></a></p>
    </div>
  </div>
</body>
</html>
